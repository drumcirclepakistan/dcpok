...
export async function registerRoutes(
  ...
  app.use(
    session({
      store: new PgStore({
        ...
        // FIX 1: SSL Support for Render sessions
  ...
  // DATABASE INITIALIZATION (Ensures all columns and Admin exist)
  try {
    console.log("Initializing database schema and fixes...");
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS band_members (
        id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
        name TEXT NOT NULL,
    ...
    // FIX 2: Add missing columns immediately to prevent startup errors
    ...
    // FIX 3: Force Create Admin if database is empty
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS shows (
        id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
        title TEXT NOT NULL,
        city TEXT NOT NULL,
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS show_expenses (
        id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS show_members (
        id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
        show_id VARCHAR NOT NULL,
        name TEXT NOT NULL,
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS show_types (
        id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      let valid = (user.password === password); // First login fallback
      if (!valid) valid = await storage.verifyPassword(password, user.password);
      ...
      if (user.role === "member") {
        ...
        return res.json({ ...safeUser, bandMemberId: bandMember?.id || null, bandMemberName: bandMember?.name || null, canAddShows: bandMember?.canAddShows || false, canEditName: bandMember?.canEditName || false, canViewAmounts: bandMember?.canViewAmounts || false, canGenerateInvoice: bandMember?.canGenerateInvoice || false });
      }
      res.json(safeUser);
  ...
  app.post("/api/auth/emergency-reset", async (req, res) => {
    try {
      ...
      if (!recoveryKey || !newPassword) return res.status(400).json({ message: "Key and password required" });
  ...
  app.get("/api/shows/:id", requireAdmin, async (req, res) => {
    ...
    if (!show || show.userId !== req.session.userId) return res.status(404).json({ message: "Not found" });
    res.json(show);
  });
  ...
  // ALL ORIGINAL EXPENSES & MEMBERS ROUTES
  app.get("/api/shows/:id/expenses", requireAdmin, async (req, res) => {
    const expenses = await storage.getShowExpenses(req.params.id);
    res.json(expenses);
  });
  ...
  app.get("/api/shows/:id/members", requireAdmin, async (req, res) => {
    const members = await storage.getShowMembers(req.params.id);
    res.json(members);
  });
  ...
  // ALL ORIGINAL STATS / DASHBOARD ROUTES
  app.get("/api/dashboard/stats", requireAdmin, async (req, res) => {
    try {
      ...
      res.json({ totalRevenue, totalExpenses, revenueAfterExpenses: totalRevenue - totalExpenses - totalPayouts, showsPerformed: filtered.length });
    } catch (err: any) { res.status(500).json({ message: "Failed" }); }
  });
  ...
  app.get("/api/financials", requireAdmin, async (req, res) => {
    try {
      const allShows = await storage.getShows(req.session.userId!);
      res.json({ shows: allShows.sort((a, b) => new Date(b.showDate).getTime() - new Date(a.showDate).getTime()) });
    } catch (err: any) { res.status(500).json({ message: "Failed" }); }
  });

  // ALL ORIGINAL BAND MEMBERS ROUTES
  ...
  // ALL ORIGINAL MEMBER-SCOPED ROUTES
  ...
  app.get("/api/member/shows", requireAuth, async (req, res) => {
    ...
    for (const show of allShows) {
      ...
      if (found) myShows.push({ ...show, myEarning: found.calculatedAmount });
    }
    res.json(myShows);
  });
