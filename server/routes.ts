...
export async function registerRoutes(
  ...
  app.use(
    session({
      store: new PgStore({
        ...
        // FIX: Changed to false to prevent the missing table.sql file error on Render
  ...
  // --- START OF DATABASE REPAIR & INITIALIZATION ---
  try {
    console.log("Initializing database schema and applying Render fixes...");

    // FIX: Manually create session table since the library auto-fix fails on Render
    ...
    // FIX: Adding missing columns reported in your Render logs
    ...
    // FIX: Adding missing show_type columns from logs
    ...
    // FIX: Force Create Admin user if database is empty
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // Support both hashed passwords and plain text (for initial admin setup)
      let valid = (user.password === password);
      if (!valid) {
         valid = await storage.verifyPassword(password, user.password).catch(() => false);
      }
  ...
  app.patch("/api/auth/change-password", requireAuth, async (req, res) => {
    try {
      ...
      const valid = await storage.verifyPassword(currentPassword, user.password).catch(() => user.password === currentPassword);
      if (!valid) {
  ...
  app.get("/api/member/dashboard", requireAuth, async (req, res) => {
    try {
      const member = await getMemberContext(req);
      if (!member) return res.status(403).json({ message: "Member access only" });
      res.json({ message: "Member dashboard active", memberName: member.name });
    } catch (err: any) {
